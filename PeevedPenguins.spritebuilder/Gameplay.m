...
- (void)touchBegan:(UITouch *)touch withEvent:(UIEvent *)event
{
    ...
    if (CGRectContainsPoint([_catapultArm boundingBox], touchLocation))
    {
        ...
        //set it's initial position inside the scoop of the catapult 34,138 is the position in the node space of the _catapultArm
        ...
        // we don't want it to rotate in the scoop
...
- (void)sealRemoved:(CCNode *)seal
{
    ...
    // make the particle effect clean itself up, once its finished
    explosion.autoRemoveOnFinish = TRUE;
    
    // place the particle effect on seals position
    explosion.position = seal.position;
    
    // add the particel effect to the same node the seal is on
...
- (void)update:(CCTime)delta
{
    ...
    {
        // if speed is below the minimum, assume this attempt is over
